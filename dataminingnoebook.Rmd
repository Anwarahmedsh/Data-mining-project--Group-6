...
# obtained from the 2017 November update of the SEER Program of the NCI, from kaggle website, URL:
...
var(dataset$Age)
```
# We have investigated missing values and outliers as they are crucial in analyzing any dataset.
...
#here we identify outliers and analyz their occurrences for specific variables of the dataset, and deleted them
```{r}
...
```
# This scatter plot shows the collarition between the tumor size attribute and the Survival months attribute, as when the tumor size is small the survival months is high which indicates that when the tumor size is small the women is more likely to live more months 


#changing the value IV in Grade attribute, so we can work with it easliy
```{r}
...
distributed. I chose the attributes survival months and age after trying multiple attributes i found they give the best results, and also they fit with our goal of clustring that we specicfied in phase 1
...
#Clustr of size=4
#this number was chosen after applying the elbow method which shows that 4 is the optimal number for clustring
```{r}
...
#here you can run the cluster with k=5 and see its graph and b-cubed results
 "B-Cubed Precision: 0.00125281884239539"
 "B-Cubed Recall: 0.00125281884239539"
```{r}
# Convert relevant columns to numeric
numeric_dataset <- dataset
numeric_dataset$Age <- as.numeric(as.character(numeric_dataset$Age))
