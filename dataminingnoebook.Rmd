...
The goal is to build a tree that distinguishes among the classes. For specificity, sensitivity, accuracy, precision and a lot of other measures. as we can see below the sensitivity of the tree says its 42% correctly identified by the classifier which is not a really perfect thing but the rest of the measures were great 
...
trainData <- dataset[indfod,]
testData <- dataset[-indfod,]
```
...
#Tree 9
60% training and 40% and gain ratio
