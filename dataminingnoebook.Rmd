...
hist(dataset$Age)
```
# A histogram showing the age distribution of women diagnosed with breast cancer. We conclude that breast cancer is more common among women whose ages range from 45 to 50 years (highest frequency).
...
```
# we can see the race is divided into three sections with their percentage ( as our dataset shows we have white, black , other). The pie chart shows which is the most Race diagnosed with breast cancer. as illustrated, the white race is the highest diagnosed race with 84.8% rate
...
o factoextra package: Extract and Visualize the Results of Multivariate Data Analyses.
o cluster package: provides methods for Cluster analysis.
o NbClust package: provides 30 indexes for determining the optimal number of clusters in data set and offers the best clustering scheme from different results.
o set.seed(): Create reproducible results
o kmeans(): Clusters data based on similarity.
o fviz_cluster(): Visualize clustering results.
o silhouette(): Find the average for each and all clusters.
o fviz_silhouette(): Visualize the silhouette information.
o fviz_nbclust(): Determining the optimal number of clusters.


# We will experiment with the following three values of k: (3,4,5). We will compute the average silhouette ,total within-cluster sum of square and the BCubed (precision and recall) for each K.
...
datawithcl<-dataset #dataset with the class label)
dataset <- dataset[, -which(names(dataset) == "Status")]

str(dataset)


#Converting interger&factor columns to numeric SO we can perform 
...
# Assuming you have a clustering result stored in Kresult$cluster
cluster_assignments <- kmns$cluster

# Calculate the majority cluster
majority_cluster <- names(which.max(table(cluster_assignments)))

# Print the majority cluster
...
# Since choosing the optimal number of clusters can help represinting the data in the best way possible and saves much time
# To find the optimal number of clusters; we have used "fviz_nbclust()" with silhouette method. it
indicates that the optimal number of clusters is K=3.
```{r}
# 2- silhouette method
